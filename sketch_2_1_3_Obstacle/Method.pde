...
void advect(int b, float[] d, float[] d0, float[] Vx, float[] Vy, float dt){
  ...
  for(i = 1; i <= Nx; i++){
    for(j = 1; j <= Ny; j++){
      //if(!isWall[IX(i, j)]){
      if(solvingV){
      x = (float)i-dt*Nx*Vx[IX(i, j)]/abs(dens[IX(i, j)]*d0[IX(i, j)]/100+1); y = (float)j-dt*Ny*Vy[IX(i, j)]/abs(dens[IX(i, j)]*d0[IX(i, j)]/100+1);
      }else{
...
void set_bnd(int b, float[] x) {
  for (int i = 1; i < Nx - 1; i++) {
    ...
    //x[IX(i, Ny/2)] = b == 2 ? -x[IX(i, Ny/2-1)] : x[IX(i, Ny/2-1)];
  }
  for (int j = 1; j < Ny - 1; j++) {
    x[IX(0, j)] = b == 1 ? -x[IX(1, j)] : x[IX(1, j)];
    x[IX(Nx-1, j)] = b == 1 ? -x[IX(Nx-2, j)] : x[IX(Nx-2, j)];
    //x[IX(Nx/2, j)] = b == 1 ? -x[IX(Nx/2-1, j)] : x[IX(Nx/2-1, j)];
  }
