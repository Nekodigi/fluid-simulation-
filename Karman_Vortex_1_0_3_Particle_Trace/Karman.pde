...
class Karman{
  ...
  void show(){
    ...
    for(int i = 0; i < 100; i++){
      particles.add(new PVector(50, random(height/2-height/4, height/2+height/4)));
    }
    for(PVector p : particles){
      int i = int(p.x/pSize);
      int j = int(p.y/pSize);
      if(i < nx){
      p.add(new PVector(u_[i][j]*pSize, v_[i][j]*pSize));
      }
      rect(p.x, p.y, 4, 4);
    }
    for(int i = particles.size()-1; i > 100000 ; i--){
      particles.remove(0);
    }
  }
